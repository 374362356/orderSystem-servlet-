			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.listfoodType}">
						<c:forEach var="food" items="${requestScope.listfoodType}">
							<tr>
								<td>${food.id}</td>
								<td>${food.typeName}</td>
								<td>
									<a href="${pageContext.request.contextPath}/foodType?id=${food.id}&method=viewUpdate" class="FunctionButton">更新</a>
									<a href="${pageContext.request.contextPath}/foodType?id=${food.id}&method=delete" class="FunctionButton">删除</a>
								</td>
							</tr>
						</c:forEach>
					</c:when>
